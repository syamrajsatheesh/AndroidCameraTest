...
class SimpleAndroidTests(unittest.TestCase):
    ...
    def db_connect(self):
        ...
        cnx = psycopg2.connect(user=str(self.db['user']), password=str(self.db['pass']), host=str(self.db['host']), database=str(self.db['name']), port=str(self.db['port']))
        cursor = cnx.cursor()
        cursor.execute("select avg(y_value) from session_timeseries_data where session_id = '{}'  AND timeseries_key = 'video_quality_mos' AND x_value between {} and {};".format(self.video_start,self.video_end))
